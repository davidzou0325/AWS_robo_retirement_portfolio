    ...
    if age is not None:
        if age < 65:
            return build_validation_result(
                False,
                'age',
                'You shoud be at least 65 years old to use this service.',
            )
...
def recommend_portfolio(intent_request):
    ...
    return close(
        ...
        "Fulfilled",
        {
            ...
            """.format(
                first_name, investment_amount, get_recommendation()
            ),
        },
    )   
def get_recommendation():
    ...
    risk_level = get_slots(intent_request)["riskLevel"]
    if risk_level =='None':
        recommendation = 'My portfolio suggestion is 100% bonds (AGG), 0% equities (SPY)'
    elif risk_level =='Low':
        recommendation = 'My portfolio suggestion is 60% bonds (AGG), 40% equities (SPY)'
    elif risk_level =='Medium':
        recommendation = 'My portfolio suggestion is 40% bonds (AGG), 60% equities (SPY)'
    elif risk_level =='High':
        recommendation = 'My portfolio suggestion is 20% bonds (AGG), 80% equities (SPY)'
